<?php
defined('BASEPATH') OR exit('No direct script access allowed');

class Dept extends CI_Controller {

	private $title = " - Portal Smart City Solo";

	public function __construct()
	{
		parent::__construct();
        $this->load->model('MDept','md');

		if (!$this->session->userdata('id')) {
            redirect('/');
		}
	}

	public function index()
	{
		$data = [
			'link' => 'departemen',
			'title' => 'Departemen'.$this->title,
            'fileScript' => 'departemen.js',
		];
		$this->load->view('main',$data);
	}

    public function dt_dept()
	{
		echo $this->md->dt_dept();
	}

    public function in_dept()
    {
		$msg = 'Gagal menambahkan data';
		$status = false;
        $ktg = '';

		$nama_dept = $this->input->post('nama_dept');
		$deskripsi = $this->input->post('deskripsi');
        $kategori_ar_id = $this->input->post('kategori_ar_id');
        foreach ($kategori_ar_id as $key => $v) {
            $ktg .= $v.',';
        }
		$gambar = $this->do_upload('gambar','./data/dept/');
   
        $obj = [
            'nama_departemen' => $nama_dept,
            'deskripsi_dept' => $deskripsi,
            'kategori_ar_id' => $ktg,
            'image' => $gambar[1],
            'ctd_date' => date('Y-m-d H:i:s')
        ];

		$in = $this->md->inDept($obj);
        if ($in != '') {

            $status = true;
            $msg = "Berhasil menambahkan data";
			
		}else{
			$status = false;
			$msg = "Gagal menambahkan data";
		}

		$response = [
			'msg' => $msg,
			'status' => $status
		];

		echo json_encode($response);
    }

    public function getDept($id='')
    {
        $data = '';
		$limit = $this->input->post('limit');
		$status = $this->input->post('status');
        if (!empty($id)) {
           $q = $this->md->get($id)->row();
        }elseif(!empty($limit)){
            $q = $this->md->get('',['status' => 1],'',$limit)->result();
        }elseif(!empty($status)){
            $q = $this->md->get('',['status' => $status])->result();
        }else{
            $q = $this->md->get()->result();
        }

        $data = [
			'data' => $q,
        ];

        echo json_encode($data);
	}

    public function up_dept()
    {
        $ktg= '';
		$id = $this->input->post('id');
		$nama_dept = $this->input->post('nama_dept');
		$deskripsi = $this->input->post('deskripsi');
        $kategori_ar_id = $this->input->post('kategori_ar_id');
        if ($kategori_ar_id != '') {
            foreach ($kategori_ar_id as $key => $v) {
                $ktg .= $v.',';
            }
        }
		$gambar = $this->do_upload('gambar','./data/dept/');

        // var_dump($gambar);die();
        
        if ($gambar[0] == 0) {
            $obj = [
                'nama_departemen' => $nama_dept,
				'deskripsi_dept' => $deskripsi,
                'kategori_ar_id' => $ktg,
                'ctd_upd' => date('Y-m-d H:i:s'),
            ];
        } else {
            $obj = [
                'nama_departemen' => $nama_dept,
				'deskripsi_dept' => $deskripsi,
                'kategori_ar_id' => $ktg,
				'image' => $gambar[1],
                'ctd_upd' => date('Y-m-d H:i:s'),
            ];
        }
		
		
		$up = $this->md->upDept($obj,['id' => $id]);
		if ($up) {
            $status = true;
            $msg = "Berhasil mengupdate data";
		}else{
			$status = false;
			$msg = "Gagal mengupdate data";
		}

		$response = [
			'msg' => $msg,
			'status' => $status
		];

		echo json_encode($response);
	}

    public function del_dept($id)
	{

		$x = $this->md->delDept(['id' => $id]);
		
		if ($x) {
			$status = true;
			$msg = "Berhasil menghapus data";
		}

		$response = [
			'msg' => $msg,
			'status' => $status
		];

		echo json_encode($response);
	}

    public function do_upload($name='',$path='')
    {
            $this->load->library('upload');
            
            $d = '';
            $s = 0;
            $msg = '';

            $config['upload_path']          = $path;
            $config['allowed_types']        = 'jpg|png|jpeg';
            $config['max_size']             = 0;
            $config['max_width']            = 0;
            $config['max_height']           = 0;

            $this->upload->initialize($config);

            if (!$this->upload->do_upload($name)){
                $msg = $this->upload->display_errors();
            }else{
                $d = $this->upload->data()['file_name'];
                $s = 1;
            }

            return [$s,$d,$msg,$this->upload->data(),$config];
    }

}
